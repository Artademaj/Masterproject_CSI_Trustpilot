...
class ReviewsSpider(scrapy.Spider):
    ...
    start_urls = ['https://www.trustpilot.com/categories/construction_manufactoring']
    ...
    def parse_details(self, response):
        for review in response.css('div.review-content'):
            yield {
                ...
                'Number of reviews': response.css('span.headline__review-count::text').extract_first(),
                'General rating': response.css('p.header_trustscore::text').extract_first(),
                'Review rating': review.xpath('div//@alt').re(r'(\d+)'),
                'Review text': " ".join(review.xpath('string(div/p[descendant-or-self::text()])').extract_first().split())
            }
